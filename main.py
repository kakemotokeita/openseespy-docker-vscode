"""
This code is modified to fit 'openseespy-docker-vscode'.
Original code, 'test_motion_dt0p01.txt' and 'opensees_constants are from openseespy example below.
https://openseespydoc.readthedocs.io/en/latest/src/exampleNamedSpacedNonlinearSDOF.html
"""
...
def show_single_comparison():
    ...
    resp_u, _, resp_a = eqsig.sdof.response_series(motion=rec, dt=motion_step, periods=periods, xi=xi)
    ...
    plt.plot(acc_signal.time, resp_u[0], label="Eqsig")
    plt.plot(outputs["time"], ux_opensees, label="Opensees fy=%.3gN" % f_yield, ls="--")
    plt.plot(outputs["time"], ux_opensees_elastic, label="Opensees fy=%.3gN" % (f_yield * 100), ls="--")
